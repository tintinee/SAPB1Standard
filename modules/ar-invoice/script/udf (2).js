...
let mainTable = sessionStorage.objectTable;
...
sessionStorage.clear();
		...
		$(document.body).on('keyup', '.inputUdf', function () 
		{
			if($(this).attr('type') == 'text' && $(this).attr('table') == ''){	
				let value = $(this).val();
				$(this).parent().parent().find('.udfcode').val(value);
			}
		});
		...
		/*UDF Date fields*/
		$(document.body).on('change', '.udfDateType', function () 
		{
	...
	$(document.body).on('change', '.selectUdf', function () 
	{
		...
		$(this).parent().parent().find('.udfcode').val(option);
