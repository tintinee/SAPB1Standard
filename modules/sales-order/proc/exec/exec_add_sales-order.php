...
$approvalCodes = $_POST['approvalCodes'];
...
if ($err == 0) 
{
	...
	else
	{
		...
		// }
			...
			// $oRdr->UserFields->Fields["U_ApprovalCodes"]->Value = $approvalCodes;
			...
			else
			{
				...
				// 	SELECT * FROM [@APR2] where Code IN ( ". $approvalCodes . " ) ");

				// while (odbc_fetch_row($qry)) 
				// {

				// 	$approvalTemplates .=  odbc_result($qry, 'TemplateCode') . ', ';
				// }

				// if($approvalTemplates != ''){
				// 	$qry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."];
				// 	UPDATE ODRF SET U_ApprovalTemplates = '$approvalTemplates' WHERE DocEntry = $docentry ");
