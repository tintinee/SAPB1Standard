...
let txtCurrency = 'PHP';
//Load from 
docNum = sessionStorage.getItem("docNum");
objType = sessionStorage.getItem("objType");
if(docNum != null){
...
cardCode2 = sessionStorage.getItem("cardCode");
...
/* $('input.itemcode').on('paste', function(e){
	...
	function PopulateFields(cardCode2){
		
		let cardCode = cardCode2; 
		
		$.getJSON('../proc/views/vw_getBPDetails.php?cardCode=' + cardCode, function (data){
		
		$.each(data, function (key, val){
		let cardName = val.CardName;
		let contactPerson = val.CntctPrsn;
		let paymentTermsCode = val.GroupNum;
		let paymentTermsName = val.PymntGroup;
		let tinNumber = val.LicTradNum;
		let contactPersonCode = val.CntctCode;
